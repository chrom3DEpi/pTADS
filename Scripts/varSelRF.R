...
#    tmport <- importance(rf)
#   importances	<- tmport[,"1"]
...
#            importances	<- tmport[,"1"]
        ...
        if((length(selected.vars) < 9) | (any(selected.vars < 1))) {
            var.simplify <- FALSE
            break
        }
...
####        }
        ...
        if(length(selected.vars) <= 9) var.simplify <- FALSE
